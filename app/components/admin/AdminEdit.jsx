...
class AdminEditForm extends React.Component {
    ...
    componentDidMount() {
        App.api('adm/admin/roles').then((roles) => {
            if (roles.length === 0) {
                ...
                let {id} = this.state;
                if (id !== 0) {
                    App.api('adm/admin/item', {id}).then((admin) => {
                        this.setState({admin});
                    })
                }
            }
    ...
    handleSubmit = () => {
        ...
        let {mobile, name, roleId, email, password} = admin;
        ...
        App.api('adm/admin/save', {'admin': JSON.stringify(admin)}).then((res) => {
            AdminProfile.clear();
    ...
    render() {
        ...
        let {mobile, name, password, roleId = 0, email, img} = admin;
        ...
        return <div className="common-edit-page">
            ...
            <Card
                ...
                <FormItem
                    {...CTYPE.formItemLayout}
                    label="邮箱">
                    <Input value={email} placeholder="请输入邮箱" style={{width: '300px'}} onChange={(e) => {
                        this.setState({
                            admin: {
                                ...admin,
